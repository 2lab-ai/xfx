...
| Surface | Kind | Status | Notes and upstream evidence |
|---|---|---|---|
| status/doctor text renderer | ui | implemented | One `[surface] key=value` line per fact. `output_contracts.zig:410-446`, `:1209-1236`. `backend` (and `backend_url` when the backend has a configured one, `backend_rejected` when it could not be read) follows `model` directly, because a model name means nothing without the endpoint it is asked of. `backend=rejected` is a fourth value alongside `gateway`/`llmux`, for a setting that named neither. |
| status/doctor JSON renderer | ui | implemented | Exactly one newline-terminated document. `output_contracts.zig:489-540`, `:1240-1285`. Carries the same `backend`/`backend_url` fields, plus `backend_rejected` quoting a `backend` value xfx could not read. On the llmux backend `auth` is `llmux-keyless-loopback` and `auth_refreshable` is false; `auth_help` is absent only when the machine can actually run a turn, and otherwise carries the refusal -- `xfx setup llmux` when no endpoint resolved, or the `backend` setting and its value when nothing validly chose a backend. A machine every turn refuses is never rendered as a healthy one. |
