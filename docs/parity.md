...
| `ask` | command | implemented | `[--auto\|--yolo] [--json] [--no-save] [--add-dir <PATH>]... <prompt>`. A bounded multi-step Gateway turn: ordered assistant text, tool calls executed locally under a permission authority, then exactly one terminal event. `--no-save`, `--add-dir`, and the permission modes have their own rows. The resume flags are not advertised and arrive with the session slice. `cli_surface.zig:61`. |
...
| permission rules and grants | persistence | partial | Typed and enforced in memory for one run: exact `tool`+`target` allow/deny rules, and session grants recorded when a user answers "always". Nothing is read from or written to settings yet, so no rule survives the process and glob-shaped grants are absent. `permissions.zig`, `session_permission_state.zig`. |
