...
| automatic command grammar | tool group | partial | Deliberately narrower than upstream's auto classifier. Reporting commands only: no `&&` chaining, no package-manager build/test families, and no Cargo subcommand that compiles or runs the workspace. Widening any of it is a review decision. `command_effect.zig:16-104`. |
