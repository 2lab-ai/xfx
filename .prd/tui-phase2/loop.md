...
| alt-screen file-diff approval | WU 7A-7B | **implemented** | QA 20 registered and green; 32/32 mutants for 7A and 61/61 for 7B, every one executed; the surface, its bound and its one-write restore are now `docs/parity.md`'s `full-screen TUI` row and `docs/architecture.md` §"One band at the bottom of the screen" |
...
| parity `/model` shared-validation sentence | WU 4 / WU 7C | **implemented** | `docs/parity.md`'s `/model` row now states which half of the validation is shared (`provider::model::model_id_problem`, the first question `ModelSelector::apply` asks) and which is not (the catalog-membership refusal, which lives where the whole catalog is) |
...
Four rows, each with the four things a deferral owes. **The controller copies these to issue #20
before issue #19 closes**; this table is the source they are copied from.
